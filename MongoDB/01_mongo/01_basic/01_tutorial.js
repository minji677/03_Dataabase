//show,use로 시작하는것 주석처리함(빨간줄)
// 데이터베이스를 tutorial로 선정하세요.
//use tutorial
/*
*insertOne()
* - 하나의 문서를 컬렉션에 추가하는 명령어
* - 형식 :db.컬렉샨먕.insertOne({키:값,...})
* insertMany
* -여러 문서를 컬렉션에 추가하는 명형어
...
*deleteOne
* -조건에 맞는 첫번째 문서를 삭제
* deleteMany를 사용할 때는 조건 없으면 전체가 삭제되므로주의필요
* -형식db, 컬렉션명, deleteOne({조건})
 */
